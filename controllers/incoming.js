...
module.exports = (req, res, next) => {
  ...
  if (hasMediaUrl) {
    client.messages
      .create({ from: 'whatsapp:+972556686869', to: incomingNumber, body: 'Media item received and is being Hydroponically assessed.' })
      .then((message) => {
    ...
    client.messages
      .create({ from: 'whatsapp:+972556686869', to: incomingNumber, body: 'Processing your Hydroponic state. Stay tuned.' })
      .then((message) => {
        res.status(200).send(JSON.stringify({ message: 'Message received.' }));
        console.log(message);
      })
